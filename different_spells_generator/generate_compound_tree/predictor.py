...
def generate_posibles(text):
    letter_list = ['a','o','u','y','i','e','h','w','v',' ','q','k','g','z','j','x','s','t']
    if 'w' not in text and 'v' not in text :
        letter_list.remove('v')
    if 'kh' not in text and ('k' not in text and 'q'  not in text):
    ...
    if 'x' not in text and 's' not in text and 'th' not in text:
...
def generate_different_spells(new_dic, eng_name, n, all_f_dic1, all_f_dic2):
    """
    :What:
    ...
    for t in eng_name:
        # if t not in letters1 and t not in ['a','o','u','y','i','e','h','w','v',' ','q','k','g','z','j','x','s']:
        if t not in letters1 and t not in possible_lets:
            letters1.append(t)
    letters1.sort()
    if n == 3:
        if (len(eng_name) - n - 1) <= 6:
            change_count = 2
        else:
            change_count = 3
        ...
        if (len(eng_name) - n - 1) <= 7:
            change_count = 2
        elif (len(eng_name) - n - 1) <= 13:
            change_count = 3
        else:
            change_count = 4
    ...
    def create_word_from_beigin(spart, tup1, f_dic1, previosprob=None, itr=0, counter=0):  
        ...
        if tup1 in f_dic1:
            for tup2 in f_dic1[tup1]:
                ...
                if (tup1, tuple(tup2[0]), tup2[1]) in new_dic:


                    if previosprob != None :
                        prob2=new_dic[(tup1, tuple(tup2[0]), tup2[1])]*previosprob
                        prob2=math.sqrt(prob2)
                        # prob2 = prob2/2
                        

                        if (prob2 > 0.005 and len(sparts) <= 15 - 3 * itr) and (len(sparts) <= counter**2) and delelte(word, letters1) == True and check_distance(eng_name, word, 'pos') == True:
                            # if spart == '*aghanez':
                            #     print(1)
                            #     print(word)
                            # print(counter)
                        ...
                        prob2 = new_dic[(tup1, tuple(tup2[0]), tup2[1])]
                        prob2=math.sqrt(prob2)
                        if (prob2 > 0.005 and len(sparts)<25) and delelte(word, letters1) == True:
                            sparts.update( {word : prob2 } )
    ...
    def create_word_from_end(epart, tup1, f_dic2, previosprob=None, itr=0, counter=0):
        eparts={}
        word=''
            ...
            for tup2 in f_dic2[tup1]:
                ...
                if (tup1, tuple(tup2[0]), tup2[1]) in new_dic:
                    ...
                    if previosprob != None :
                        ...
                        prob2=math.sqrt(prob2)
                        # prob2 = prob2/2
                        if (prob2 > 0.005 and len(eparts) <= 15 - 3 * itr) and (len(eparts) <= counter**2) and delelte(word, letters1) == True and check_distance(eng_name, word, 'neg') == True:
                        ...
                        prob2 = new_dic[(tup1, tuple(tup2[0]), tup2[1])]
                        prob2=math.sqrt(prob2)
                        if (prob2 > 0.005 and len(eparts)<35) and delelte(word, letters1) == True:
                            eparts.update( {word : prob2 } )
    ...
    new_neg_parts=create_word_from_end('', ng[-1], all_f_dic2)
    new_pos_parts=create_word_from_beigin('', ng[0], all_f_dic1)


    def my_func(parts, n, dic, x, itr):
        ...
        for part, value in parts.items():
            ...
            if '*' in part and '#' in part :
                ...
                if x=='pos' :
                    last_tup=list(ngrams(part,n))[-1]
                    ans=create_word_from_beigin(part[:-n], last_tup, dic, value, itr, len(parts) - counter)
                    counter += 0
                    
                    # if '*aghanezhad#' in ans:
                    #     print('khar')
                elif x=='neg' :
                    first_tup=list(ngrams(part,n))[0]
                    ans=create_word_from_end(part[n:], first_tup, dic, value, itr, len(parts) - counter)
                    counter += 0
    ...
    if int((len(eng_name) - n -1)/(change_count)) +1 <= 5:
        loop_count = 4
    else :
        loop_count = 5 
    loop_count 
    # if int((len(eng_name)-n -1)/change_count)
    for inn1 in range(loop_count):
        new_pos_parts={k: v for k, v in sorted(new_pos_parts.items(), key=lambda item: item[1],reverse=True)}
        new_pos_parts = my_func(new_pos_parts, n, all_f_dic1, 'pos', inn1)
        if len(new_pos_parts)>15000:
            break
        # print(len(new_pos_parts))

    for inn2 in range(loop_count):
        new_neg_parts={k: v for k, v in sorted(new_neg_parts.items(), key=lambda item: item[1],reverse=True)}
        new_neg_parts = my_func(new_neg_parts, n, all_f_dic2, 'neg', inn2)
...
def fileter(orginal_persian_name, dic_of_word, entext):
    ...
    equivalent_f = {'م':['m'], 'ر':['r'], 'ن':['n'],'ش':['sh'], 'س':['s'], 'ص':['s'], 'ث':['s','th'], 'پ':['p'], 'ت':['t'], 'ط':['t'], 'ز':['z'], 'ض':['z'], 'ظ':['z'], 'ذ':['z'],'ژ':['zh', 'j'], 'ج':['g','j'],'ف':['f'], 'د':['d'],'ل':['l'],  'ک':['k', 'q','ch'],'ب':['b'],'ق':['gh','q','g'],'غ':['gh'],'گ':['g'],'خ':['kh']}
...
def translate_per_to_en(text, source, target):
    try:
        ...
        translated = GoogleTranslator(
            source="fa", target="en", proxies=PROXIES
        ).translate(text)
...
def tranlator_filter(fars_name, spells):
    ...
    for text2tr in text_to_translate_list:
        ...
        for trtext1 in (translated1.split("\n")):
            ...
            if translated_splited_from_rezaeifar== fars_name or translated_splited_from_rezaeifar == fars_name_no_space or  fars_name == no_space_translated or fars_name_no_space == no_space_translated:
...
def get_ready_to_generate_different_spelling(fatext, translated, type_name):
    ...
    if type_name == "Last_name":
        ...
        eng_name = translated.lower().split(" ahmadi")[0]
        ...
        for ename in eng_name.split():
            if len(ename) <= 5 :
                [new_pos_parts, new_neg_parts, letters] = generate_different_spells(
                    probility_firstname2,
                    "*" + ename + "#",
                    NGRAM_COUNTS[0],
                    left_side_firstname2,
                    right_side_firstname2,
                )
                annot_names = recognize_true_different_spells(
                    "*" + ename + "#", fatext, new_pos_parts, new_neg_parts, letters
                )
                ...
                [new_pos_parts, new_neg_parts, letters] = generate_different_spells(
                    probility_firstname3,
                    "*" + ename + "#",
                    NGRAM_COUNTS[1],
                    left_side_firstname3,
                    right_side_firstname3,
                )
                annot_names = recognize_true_different_spells(
                    "*" + ename + "#", fatext, new_pos_parts, new_neg_parts, letters
                )
                ...
                [new_pos_parts, new_neg_parts, letters] = generate_different_spells(
                    probility_firstname4,
                    "*" + ename + "#",
                    NGRAM_COUNTS[2],
                    left_side_firstname4,
                    right_side_firstname4,
                )
                annot_names = recognize_true_different_spells(
                    "*" + ename + "#", fatext, new_pos_parts, new_neg_parts, letters
                )
    ...
    result = tranlator_filter(fars_name=fatext, spells=result)
...
def get_name(persian_fname, persian_lname):
    ...
    return final_firstname_list, final_lastname_list, list(different_spells.keys())[:100], seyed_flag
    # return list(fspells.keys())[:5], list(lspells.keys())[:5], 1

# print(get_name('آتنا','سراج زاده'))
